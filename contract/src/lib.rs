...
impl FarmingContract {
    ...
    pub fn storage_deposit(&mut self) {
        ...
        self.storage_deposits
            .insert(&account_id, &(current + attached_deposit.as_yoctonear()));
    }
    ...
    fn update_farm(&mut self, farm_id: u64) {
        ...
        for i in 0..farm.reward_tokens.len() {
            ...
            let potential_reward =
                (sessions_elapsed as u128).saturating_mul(farm.reward_per_session[i]);
            ...
            if reward_to_distribute > 0 {
                ...
                farm.remaining_reward[i] =
                    farm.remaining_reward[i].saturating_sub(reward_to_distribute);
            }
        }
        ...
        if current_time >= sim.start_time && sim.total_staked > 0 {
            ...
            if sessions_elapsed > 0 {
                for i in 0..sim.reward_tokens.len() {
                    let potential_reward =
                        (sessions_elapsed as u128).saturating_mul(sim.reward_per_session[i]);
    ...
    fn stake_tokens(
        ...
        assert_eq!(
            farm.status,
            FarmStatus::Active,
            "Farm is ended, staking not allowed"
        );

        assert_eq!(
            farm.staking_token, token_in,
            "Not the correct staking token"
        );
    ...
    pub fn claim_rewards(&mut self, farm_id: u64) {
        ...
        env::log_str(format!("User {} claimed all rewards in farm {}", user, farm_id).as_str());
    }
...
mod tests {
    ...
    fn test_storage_deposit_and_create_farm() {
        ...
        let mut contract = FarmingContract::new();
    ...
    fn test_create_farm_insufficient_storage_multitoken() {
        ...
        let input = FarmInput {
            staking_token: "staking.token".parse().unwrap(),
            reward_tokens: vec![
                "reward1.token".parse().unwrap(),
                "reward2.token".parse().unwrap(),
            ],
            lockup_period_sec: 60,
    ...
    fn test_staking_flow() {
        ...
        testing_env!(context.build());
        contract.ft_on_transfer(AccountId::try_from(accounts(0)).unwrap(), U128(500), msg);
    ...
    fn test_stake_insufficient_storage() {
        ...
        contract.ft_on_transfer(AccountId::try_from(accounts(1)).unwrap(), U128(100), msg);
    }
    ...
    fn test_add_reward() {
        ...
        testing_env!(context.build());
        contract.ft_on_transfer(AccountId::try_from(accounts(0)).unwrap(), U128(10_000), msg);
    ...
    fn test_session_based_distribution() {
        ...
        context = get_context("staking.token".parse().unwrap(), 0, 0);
        testing_env!(context.build());
        contract.ft_on_transfer(AccountId::try_from(accounts(0)).unwrap(), U128(100), msg);
