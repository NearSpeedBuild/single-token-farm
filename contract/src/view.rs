...
impl From<(&FarmParams, u64)> for FarmView {
    fn from((farm, farm_id): (&FarmParams, u64)) -> Self {
        FarmView {
            ...
            reward_per_session: farm.reward_per_session.iter().map(|v| U128(*v)).collect(),
...
impl FarmingContract {
    ...
    pub fn get_stake_info(&self, account_id: AccountId, farm_id: u64) -> Option<StakeInfoView> {
        ...
        if let Some(info) = self.stakes.get(&key) {
            if let Some(farm) = self.farms.get(&farm_id) {
                ...
                let updated_accrued: Vec<U128> = info
                    .accrued_rewards
                    .iter()
                    .enumerate()
                    .map(|(i, &val)| {
